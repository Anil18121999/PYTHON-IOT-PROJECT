...
" Unit:"))
    ...
    def load_data(self):
        ...
        connection = sqlite3.connect("tyr.db") 
        results = connection.execute("SELECT SPECIMEN_NAME, JOB_NAME, BATCH_ID, MOTOR_SPEED, MOTOR_REV_SPEED, OPERATOR, GRAPH_SCAL_Y_LOAD, IFNULL(GRAPH_SCAL_X_LENGTH, 0), LOAD_POINTS, IFNULL(NEW_TEST_MAX_LENGTH, 0), IFNULL(NEW_TEST_MAX_LOAD, 0) LAST_UNIT_LOAD, LAST_UNIT_DISP FROM TEST_MST WHERE TEST_ID = '"+str(self.label_12.text())+"'") 
        for column in results:
