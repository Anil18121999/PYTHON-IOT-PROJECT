...
# Form implementation generated from reading ui file 'TYR_TEST_REPORT_IFD.ui'
#
...
" Unit:"))
    ...
    def load_data(self):
        ...
        self.comboBox.clear()   
        connection = sqlite3.connect("tyr.db")
        results = connection.execute("SELECT LOAD_POINTS, D1_PER, D1, D2_PER, D2, D3_PER, D3, D4_PER, D4 FROM TEST_DATA_RADIAL WHERE TEST_ID = '"+str(self.label_12.text())+"'") 
        for x in results:
        ...
        connection = sqlite3.connect("tyr.db") 
        results = connection.execute("SELECT SPECIMEN_NAME, MOTOR_REV_SPEED, MOTOR_SPEED, PRE_LOAD, WIDTH, IFNULL(THICKNESS, 0), GUAGE_LENGTH, JOB_NAME, BATCH_ID, LAST_UNIT_LOAD, GRAPH_SCAL_X_LENGTH, GRAPH_SCAL_Y_LOAD, IFNULL(GRAPH_SCAL_Y_LOAD_N, 0)  FROM TEST_MST WHERE TEST_ID = '"+str(self.label_12.text())+"'") 
    ...
    def create_pdf_Tear(self):
        ...
        if(str(self.comboBox.currentText()) == "03"):  
              ...
              connection.close()
              connection = sqlite3.connect("tyr.db")
              results=connection.execute("SELECT 'Avg', printf(\"%.2f\",avg(STIFFNESS)), printf(\"%.2f\",avg(D1)), printf(\"%.2f\",avg(D2)), printf(\"%.2f\",avg(D3)) FROM TEST_DATA_RADIAL WHERE TEST_ID='"+str(int(self.label_12.text()))+"'")
              for y in results:
                        data2.append(y)
                        # print(data2)
              connection.close()
              ...
              connection = sqlite3.connect("tyr.db")
              results=connection.execute("SELECT 'Max', printf(\"%.2f\",Max(STIFFNESS)), printf(\"%.2f\",max(D1)) ,printf(\"%.2f\",max(D2)),printf(\"%.2f\",max(D3)) FROM TEST_DATA_RADIAL WHERE TEST_ID='"+str(int(self.label_12.text()))+"'")
              for z in results:
                        data2.append(z)
              connection.close()
                  
              connection = sqlite3.connect("tyr.db")
              results=connection.execute("SELECT 'Min', printf(\"%.2f\",Min(STIFFNESS)), printf(\"%.2f\",min(D1)) ,printf(\"%.2f\",min(D2)), printf(\"%.2f\",min(D2)) FROM TEST_DATA_RADIAL WHERE TEST_ID='"+str(int(self.label_12.text()))+"'")
              for n in results:
                        data2.append(n)
              connection.close()
              ...
              connection = sqlite3.connect("tyr.db")
              results=connection.execute("SELECT 'Avg', printf(\"%.2f\",avg(STIFFNESS)), printf(\"%.2f\",avg(D1)) , printf(\"%.2f\",avg(D2)), printf(\"%.2f\",avg(D3)), printf(\"%.2f\",avg(D4)) FROM TEST_DATA_RADIAL WHERE TEST_ID='"+str(int(self.label_12.text()))+"'")
              for y in results:
                        data2.append(y)
                        # print(data2)
              connection.close()
              ...
              connection = sqlite3.connect("tyr.db")
              results=connection.execute("SELECT 'Max', printf(\"%.2f\",Max(STIFFNESS)), printf(\"%.2f\",max(D1)) ,printf(\"%.2f\",max(D2)), printf(\"%.2f\",max(D3)), printf(\"%.2f\",max(D4)) FROM TEST_DATA_RADIAL WHERE TEST_ID='"+str(int(self.label_12.text()))+"'")
              for z in results:
                        data2.append(z)
              connection.close()
                  
              connection = sqlite3.connect("tyr.db")
              results=connection.execute("SELECT 'Min', printf(\"%.2f\",Min(STIFFNESS)), printf(\"%.2f\",min(D1)) ,printf(\"%.2f\",min(D2)), printf(\"%.2f\",min(D3)), printf(\"%.2f\",min(D4)) FROM TEST_DATA_RADIAL WHERE TEST_ID='"+str(int(self.label_12.text()))+"'")
              for n in results:
                                data2.append(n)
              connection.close()
        else:
              ...
              results = connection.execute("SELECT SPEC_ID, IFNULL(STIFFNESS, 0) FROM TEST_DATA_RADIAL WHERE TEST_ID = '"+str(self.label_12.text())+"'")
              for x in results:
                        data2.append(x)
                        print(data2)
              connection.close()
              connection = sqlite3.connect("tyr.db")
              results=connection.execute("SELECT 'Avg', printf(\"%.2f\",avg(STIFFNESS)), printf(\"%.2f\",avg(D1)) , printf(\"%.2f\",avg(D2)) FROM TEST_DATA_RADIAL WHERE TEST_ID='"+str(int(self.label_12.text()))+"'")
              for y in results:
                        data2.append(y)
                        # print(data2)
              connection.close()
              ...
              connection = sqlite3.connect("tyr.db")
              results=connection.execute("SELECT 'Max', printf(\"%.2f\",Max(STIFFNESS)), printf(\"%.2f\",max(D1)) ,printf(\"%.2f\",max(D2)) FROM TEST_DATA_RADIAL WHERE TEST_ID='"+str(int(self.label_12.text()))+"'")
              for z in results:
                        data2.append(z)
              connection.close()
                  
              connection = sqlite3.connect("tyr.db")
              results=connection.execute("SELECT 'Min', printf(\"%.2f\",Min(STIFFNESS)), printf(\"%.2f\",min(D1)) ,printf(\"%.2f\",min(D2)) FROM TEST_DATA_RADIAL WHERE TEST_ID='"+str(int(self.label_12.text()))+"'")
              for n in results:
                        data2.append(n)
              connection.close()
